...
export default function LightSection({ secData }: {secData: secProps}) {
  return (
    <section className={`w-full ${secData.reverse && "bg-[#f0efed]/40"}`}>
      <div className={`max-ctn px-4 py-24 ${secData.servicePage && "py-40 max-sm:py-16"} sm:px-6 lg:px-8 m-auto`}>
          ...
          <div className=" w-full max-w-xl lg:mt-0">
            <div className="space-y-6 sm:space-y-8">
              <div className="space-y-2 md:space-y-4">
                <h1 className={`font-bagero text-[#5d6b48]  ${secData.servicePage ? "text-5xl max-sm:text-[25px]" : "text-7xl max-sm:text-[50px]"}`}>
                  {secData.title}
                </h1>
